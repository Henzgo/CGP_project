...
void display(GLFWwindow* window, double currentTime) {
    ...
    glm::vec3 offset = glm::vec3(-cameraX, -cameraY, -cameraZ);

    tMat = glm::translate(identity, offset);

    rMat = glm::mat4(1.0f);
    rMat[0][0] = cameraU.x; rMat[1][0] = cameraU.y; rMat[2][0] = cameraU.z;
    rMat[0][1] = cameraV.x; rMat[1][1] = cameraV.y; rMat[2][1] = cameraV.z;
    rMat[0][2] = -cameraN.x; rMat[1][2] = -cameraN.y; rMat[2][2] = -cameraN.z;
...
void cursor_position_callback_manually(GLFWwindow* win, double xpos, double ypos) {
    if (isDragging) {
        ...
        cameraN = glm::rotate(cameraN, angleX, cameraV);
        cameraU = glm::rotate(cameraU, angleX, cameraV);

        cameraN = glm::rotate(cameraN, angleY, cameraU);
        cameraV = glm::rotate(cameraV, angleY, cameraU);

        cameraN = glm::normalize(cameraN);
        cameraU = glm::normalize(glm::cross(cameraN, cameraV)); // damit Orthogonalität erhalten bleibt.
        cameraV = glm::normalize(glm::cross(cameraU, cameraN));

        //std::cout << "deltaX: " << deltaX << ", deltaY: " << deltaY << std::endl;

        // Speichern aktuelle Mausposition für den nächsten Callback
        lastMouseX = xpos;
        lastMouseY = ypos;
    }
    else {
...
int main(void) {
    ...
    glfwSetCursorPosCallback(window, cursor_position_callback_manually);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS)
            cameraX += cameraN.x * moveSpeed,
            cameraY += cameraN.y * moveSpeed,
            cameraZ += cameraN.z * moveSpeed;
        ...
        if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS)
            cameraX += -cameraU.x * moveSpeed,
            cameraY += -cameraU.y * moveSpeed,
            cameraZ += -cameraU.z * moveSpeed;
